...
const sumLength = (sum, file) => sum + file.length;

const ensure = (bool, fieldName) => {
 ...
 */
const decodeTorrentFile = async (torrent) => {
  ...
  if (Array.isArray(torrent["announce-list"]) && torrent["announce-list"].length > 0) {
    torrent["announce-list"].forEach(urls => urls.forEach(url => result.announce.push(arr2text(url))));
 ...
 * @return {Object}
 */
const parseTorrent = async (torrentId) => {
